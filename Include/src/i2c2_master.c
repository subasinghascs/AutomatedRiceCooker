...
{
	if (i2c2_start(address | I2C_WRITE)) return 1;
	
	for (uint16_t i = 0; i < length; i++)
	{
		if (i2c2_write(data[i])) return 1;
	}
...
{
	if (i2c2_start(address | I2C_READ)) return 1;
	
	for (uint16_t i = 0; i < (length-1); i++)
	{
		data[i] = i2c2_read_ack();
	}
	data[(length-1)] = i2c2_read_nack();
...
{
	if (i2c2_start(devaddr | 0x00)) return 1;
...
{
	if (i2c2_start(devaddr)) return 1;

	i2c2_write(regaddr);

	if (i2c2_start(devaddr | 0x01)) return 1;
